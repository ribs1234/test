...
<html lang="en">
  ...
  <body>
    ...
    <p class="hint">
      Enter your Confluence web address and credentials, then search by text.
      This app calls Confluence from your local machine and does not store credentials.
    </p>
    ...
    <form id="search-form">
      ...
      <label>
        API Token (for API token auth)
        <input id="api-token" name="api_token" type="password" />
      </label>

      <label class="full">
        Bearer Token (optional alternative auth)
    ...
    def do_POST(self) -> None:  # noqa: N802 (BaseHTTPRequestHandler API)
        ...
        try:
            agent = ConfluenceSearchAgent(
                ...
                bearer_token=str(payload.get("bearer_token", "")).strip() or None,
            )
